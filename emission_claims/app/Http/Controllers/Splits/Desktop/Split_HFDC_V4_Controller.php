...
class Split_HFDC_V4_Controller extends Controller
{
    ...
    { 
        ...
        $strThankyouPage        =  env('APP_URL').$this->redirectInterface->redirectToPage('HFDC_V4_final_index',array("userId"=>$intUserId,"visitorId"=>$intVisitorId,"isQualified"=>$isQualified));
        ...
        if (isset($advRedirectDomain) && !empty($advRedirectDomain) && !empty($pid)) { 
            ...
            $this->redirectInterface->redirectToSignPage('HFDC_V4_final_index',array("userId"=>$intUserId,"visitorId"=>$intVisitorId,"isQualified"=>$isQualified));
            //$this->redirectInterface->redirectToSignPage('web/thankyou',array("userId"=>$intUserId,"visitorId"=>$intVisitorId));
        }
    }

    public function final_index( Request $request ,CommonSplitsInterface $commonsplit )
    { 
        $splitName              = 'HFDC_V4.php';
        $split_name             = str_replace('.php','',$splitName);
        ...
        $resourcePath           = asset('assets/'.$split_name).'/';
        $actionpage             = route('Split_'.$split_name.'.final_store');
        $data['title']          = 'Hausfeld';
        $data['APP_URL']        = env('APP_URL');
        $data['userId']         = $userId;
        $data['intVisitorId']   = $intVisitorId;
        ...
        return view('splits.web.'.$split_name.'.final_index', compact('data', 'resourcePath', 'actionpage' ));
    
    }

    public function final_store( Request $request, UserInterface $userRepo)
    { 
